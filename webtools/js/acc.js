...
const NON_ALPHANUM = /[^a-zA-Z0-9]/g, A = 'A'.charCodeAt(0), Z = 'Z'.charCodeAt(0);
...
function toIBAN(ctry, bban, human = false) {
  ...
  const remainder = iso7064Mod97_10(iso13616Prepare(ctry + '00' + bban));
  const checkDigit = ('0' + (98 - remainder)).slice(-2);
  const iban = ctry + checkDigit + bban;
...
function BBANok(ctry, bban) {
  if (!BBANformats.has(ctry)) {
    return null;
  };
  if (!bban.match(BBANformats.get(ctry).re)) {
    return false;
  };
...
function BBANcd(ctry, bban) {
  switch (ctry) {
    ...
    case 'ES': try { // CD1 = weighted CD on bank & branch code - CD2 weighted CD on acc nbr -- then 11 - MOD 11
      let s1 = 0; let s2 = 0; let c1; let c2;
      [4, 8, 5, 10, 9, 7, 3, 6].forEach((w, i) => {
        s1 += w * parseInt(bban[i]);
      });
      [1, 2, 4, 8, 5, 10, 9, 7, 3, 6].forEach((w, i) => {
        s2 += w * parseInt(bban[i + 10]);
      });
      c1 = 11 - (s1 % 11);
      c2 = 11 - (s2 % 11);
      if (c1 > 9) {
    ...
    case 'PT': try { // CD = weighted CD on bank & branch code & account -- then 97 - MOD 97
      ...
      weights.forEach((w, i) => {
        sum += w * parseInt(bban[i]);
      });
      const cd = String(98 - (sum % 97)).padStart(2, '0');
...
function BBANex(ctry) {
  ...
  if (ctry == 'NL') { // The Dutch acc nbr doesn't have a seperate CD but a control on acc nbr
    while (BBANcd('NL', 'ABNA' + acc) == null) {
      acc = ranTxt(f.alen, '01234567890');
    }
  }
  bban = '0'.repeat(f.len);
